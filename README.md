...
The index's possible range is from 0 to 1, and it is an average of four subindices. For each subindex, 1 would indicate equality (or female advantage). The highest score is .877 in Iceland, and the lowest score among PISA countries is .599 in Saudi Arabia (the lowest of all rated is Yemen with .494). Spearman's correlation coefficient will be used here because non-parametric statistics are most suitable.
